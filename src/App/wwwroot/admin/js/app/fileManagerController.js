var fileManagerController = function (dataService) {
    ...
    function remove() {
        ...
        for (i = 0; i < items.length; i++) {
            if (i + 1 < items.length) {
                dataService.remove('api/assets/remove?url=' + encodeURIComponent(items[i].id), emptyCallback, fail);
            }
            else {
                dataService.remove('api/assets/remove?url=' + encodeURIComponent(items[i].id), removeCallback, fail);
            }
        }
    }
    ...
    function loadCallback(data) {
        ...
        $.each(assets, function (index) {
            ...
            var tag = '<div class="col-sm-6 col-md-4 col-lg-3">' +
                '	<div class="item">' +
                '		<div class="item-img" onclick="fileManagerController.pick(\'' + asset.url + '\'); return false"><img src="' + src + '" alt="' + asset.title + '" title="' + asset.title +'" /></div>' +
                ...
                '			<span class="custom-control-label" title="' + asset.title+'">' + asset.title + '</span>' +
                '		</label>' +
                '	</div>' +
                '</div>';
